...
public class GeRoleController {
	...
	public String deleteRole(Model m, int roleId) {
	...
	public String pupdateRole(Model m, int roleId) {
		...
		m.addAttribute("roleMenuList", l1);
		return "/resource/demo3/update.jsp";
	}

	@RequestMapping("/update")
	public String updateRole(Model m, int roleId, RoleBean rb, List<Integer> menuIds) {
		rb.setRoleId(roleId);
		rms.delete(roleId);
		rs.updateRoleBy(rb);
		/*char[] mids = menuIds.toCharArray();
		List<Integer> ls = new ArrayList<Integer>();
		for (int i = 0; i < mids.length; i++) {
			ls.add(Integer.parseInt(mids[i] + ""));
		}*/
		rms.addMenu(roleId, menuIds);
		return "rolelist";
	}

	@RequestMapping("/look")
	public String lookrole(Model m, int roleId) {
		RoleBean rb = rs.getRole(roleId);
		m.addAttribute("rb", rb);

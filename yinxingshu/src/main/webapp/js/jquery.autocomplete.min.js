...
(function(g) {
	...
	var i = function(q) {
		...
		if (g
				.inArray(this.option.listStyle, [ "normal", "iconList",
						"custom" ]) == -1) {
	...
	var c = function() {
		var q = this.searchView.find("li.selected");
		if (q.size()) {
			this.inputView.val(q.attr("val"));
			e.apply(this)
		}
	};
	var a = function(q) {
	...
	var h = function() {
		var s = this, r = this.inputView.val(), t = [], q = [];
		if (g.trim(r).length == 0) {
			e.apply(s);
			return
		}
		if (g.isArray(this.option.data)) {
			...
			if (g.isFunction(this.option.data)) {
				t = this.option.data.apply(this, [ r ])
			} else {
				if (typeof (this.option.data) === "string") {
					try {
						t = a.apply(this, [ r ])
					} catch (u) {
						o.apply(this, [ "Ajax错误:" + u ]);
						return
					}
				} else {
		...
		g.each(t, function(v, x) {
			if (s.option.maxItems > 0 && q.length >= s.option.maxItems) {
				return false
			}
			if (g.isPlainObject(x)) {
				var w = g.extend(false, {}, x)
			} else {
				if (typeof (x) === "string") {
					var w = {
						"label" : x,
					...
					o.apply(s, [ "数据源Item类型错误！" ]);
					return false
				}
			}
			if (s.option.matchHandler.apply(s, [ r, w ])) {
				q.push(w)
			}
		});
		if (r == this.inputView.val()) {
			if (q.length > 0) {
				l.apply(this, [ q ])
			} else {
				e.apply(this)

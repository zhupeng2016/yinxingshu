...
<body onload="show()">
		...
		<span>
			<form action="menu/menulist?pageNum=1" method="post" >
				菜单名称: <input type="text" name="menuName" id="mn"
					value="${sessionScope.menuBean.menuName }" /> 状态: <select
					name="menuState" id="mstate">
					<option value="999">-请选择-</option>
					<option value="0"
						<c:if test="${sessionScope.menuBean.menuState==0}">selected</c:if>>启用</option>
					<option value="1"
						<c:if test="${sessionScope.menuBean.menuState==1}">selected</c:if>>禁用</option>
				</select>&nbsp;&nbsp;&nbsp; <input value="查询" type="submit" />&nbsp; <a
					href="menu/menulist?method=clear"> <input type="button"
					value="清空" /></a>
			</form>
		</span>
	</div>
		...
		<table class="table_a" border="1" width="100%">
			<tbody>
				...
				<c:forEach items="${mls }" var="ml" varStatus="st">
					<tr id="product1">
						<td><input type="checkbox" name="chosen"
							value="${ml.menuId }" /></td>
						<td>${st.index+1 }</td>
						<td>${ml.menuName }</td>
						<td><a href="view.html">${ml.url }</a></td>
						<td><c:if test="${ml.menuState ==0 }">启用</c:if> <c:if
								test="${ml.menuState ==1 }">禁用</c:if></td>
						<td><a
							href="menu/look?menuId=${ml.menuId }&pageNum=${page.getPageNum() }">查看</a>
							&nbsp;&nbsp;&nbsp; <a
							href="menu/pupdate?menuId=${ml.menuId }&pageNum=${page.getPageNum() }">修改</a>
							&nbsp;&nbsp;&nbsp; <a
							href="menu/delete?menuId=${ml.menuId }&pageNum=${page.getPageNum() }">删除</a></td>
					</tr>
				</c:forEach>
 ...
 -->
				<tr>
					<td colspan="20" style="text-align: center;">
						...
						style="text-decoration: none;" href="#"> 首页 上一页 ... 7 8 9 10
							11 12 ... 下一页 尾页 共1234条 每页显示 10/23 </a> --> <span> <a
							href="menu/menulist?pageNum=1">首页</a> <c:if
								test="${requestScope.page.getPageNum() == 1 }">
				上一页
			</c:if> <c:if test="${requestScope.page.getPageNum() > 1 }">
								<a
									href="menu/menulist?pageNum=${requestScope.page.getPageNum()-1 }">上一页</a>
							</c:if> &nbsp;&nbsp; <c:forEach
				...
				下一页
								...
								test="${requestScope.page.getPageNum() < requestScope.page.getPages() }">
								<a
									href="menu/menulist?pageNum=${requestScope.page.getPageNum()+1 }">下一页</a>
							</c:if> <a href="menu/menulist?pageNum=${requestScope.page.getPages() }">尾页</a>
